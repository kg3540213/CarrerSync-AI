...
const RoadmapGeneratorBanner = () => {
  ...
  const [showLoginPrompt, setShowLoginPrompt] = useState(false); // ← was missing
  ...
  return (
    <section className="relative bg-gray-950 overflow-hidden min-h-screen flex items-center">
      ...
      <div className="absolute inset-0 z-0 opacity-20">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,#3f3f46_1px,transparent_1px)] bg-[size:20px_20px]" />
      </div>
      ...
      >
        ...
        >
          Our intelligent system crafts tailored learning paths based on your goals, skills,
          and preferences to maximise your success.
        </motion.p>
        ...
        {showLoginPrompt && (
          <motion.div
            ...
            className="mt-6 bg-gray-800 border border-gray-700 rounded-lg p-4 max-w-md mx-auto"
          >
            <p className="mb-3 text-gray-200">Please log in to access the roadmap generator.</p>
            <div className="flex gap-2 justify-center">
              <button
                onClick={handleLoginClick}
                className="px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 rounded-md font-medium text-white"
              >
                Login
              </button>
              <button
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        {['🧠', '📊', '🚀', '🔮'].map((icon, i) => (
          <motion.div
            ...
            animate={{ y: [0, -20, 0], x: [Math.random() * 10, Math.random() * 10] }}
            transition={{
